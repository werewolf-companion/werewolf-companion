...
module.exports = class H extends client.commands.class {
    constructor() {
        super({
            ...
            description: 'Quick access to answers of frequently asked questions.',
    ...
    async run({ message, args, user }) {
        if (args[0] === 'all') return message.send({ message, title: 'Help Commands', description: Object.entries(h).map(a => `\`${a[0]}\``).join('\n') });
            ...
            const { title, value } = h[args[0]?.toLowerCase()] || {};
            if (title) {
                message.delete();
                return message.send({ title, description: value.replace(/{{prefix}}/gi, message.prefix) });
            } else return;
        }
    }
}