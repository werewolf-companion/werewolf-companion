module.exports = class Jobs extends client.commands.class {
    ...
    async run({ message, args, user }) {
        ...
        return message.send({ message, title: 'Avaiable Jobs', description: `Apply for one of these jobs by using \`${message.prefix}job apply <job title>\`.\nView your current daily streak and hours worked by using \`${message.prefix}user\`.` }, fields)
    }
}
